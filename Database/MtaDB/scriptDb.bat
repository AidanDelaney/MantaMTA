...
echo -------------------------------------------------->> scriptDb.log
echo scriptDb.bat run on %date% at %time%>> scriptDb.log
echo SQLServer:	%sqlServer%, DatabaseName: %databaseName%, OutputDirectory: %outputDir%>> scriptDb.log


echo.
echo ==============================
echo Scripting Database "%databaseName%" and Data...
echo ==============================
echo.
...
ScriptDb.exe -con:server=%sqlServer%;database=%databaseName%;trusted_connection=yes -outDir:%outputDir% -d -v -p -TableOneFile -ScriptAsCreate -CreateOnly -Permissions
