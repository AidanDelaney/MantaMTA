...
namespace MantaMTA.Core.Client.BO
{
	...
	internal class MtaQueuedMessage : MtaMessage
	{
		...
		{
			// Log the deferral.
			await MtaTransaction.LogTransactionAsync(MessageID, TransactionStatus.Deferred, defMsg, ipAddress, mxRecord);
		...
		{
			await MtaTransaction.LogTransactionAsync(this.MessageID, TransactionStatus.Success, string.Empty, ipAddress, mxRecord);
			return true;
		}
		...
		{
			// Log deferral
			await MtaTransaction.LogTransactionAsync(MessageID, TransactionStatus.Throttled, string.Empty, ipAddress, mxRecord);
		...
		{
			// Log deferral
			await MtaTransaction.LogTransactionAsync(MessageID, TransactionStatus.Deferred, "Service Unavailable", ipAddress, null);
		...
		{
			await MtaTransaction.LogTransactionAsync(MessageID, TransactionStatus.Failed, failMsg, ipAddress, mxRecord);
